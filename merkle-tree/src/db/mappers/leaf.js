...
const publishLeaf = async (leafValue, leafIndex) => {
  ...
  try {
    ...
    //   // console.log(commitmentValue);
    //   // const commitmentIndex = Math.floor(Math.random() * 4).toString();
    //   // console.log(commitmentIndex);
    //   const msg = 'Some String';
    const msg = JSON.parse(`{"${leafValue}":"${leafIndex}"}`);
    //   // console.log('Message being published', JSON.stringify(msg));
...
export default function(treeHeight, { value, nodeIndex, leafIndex, blockNumber }) {
  ...
  if (!nodeIndex) {
    ...
    // const publishLeafZero = await publishLeaf({ value, leafIndex });
    // await publishLeaf(value, leafIndex);
    publishLeaf(value, leafIndex);
    // logger.debug(`Publish Leaf Zero ${publishLeafZero}`);
    logger.debug(`TESTING CHANGES!!!!!!!`);
    logger.debug(`TESTING CHANGES MORE!!!!!!!`);
